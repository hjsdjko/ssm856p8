	...
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">驾照类型</view>
				<picker @change="jiazhaoleixingChange" :value="jiazhaoleixingIndex"  :range="jiazhaoleixingOptions">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(51, 153, 153, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.jiazhaoleixing?ruleForm.jiazhaoleixing:"请选择驾照类型"}}</view>
				</picker>
			</view>
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">学车费用</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.19)","borderColor":"rgba(51, 153, 153, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.xuechefeiyong" v-model="ruleForm.xuechefeiyong" placeholder="学车费用"></input>
			</view>
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
			...
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"308rpx"}' class="cu-form-group">
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">费用说明</view>
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(51, 153, 153, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.feiyongshuoming" placeholder="费用说明"></textarea>
			</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			this.ruleForm.jiaxiaodizhi = this.user.jiaxiaodizhi
			this.ro.jiaxiaodizhi = true;
			this.ruleForm.tupian = this.user.tupian
			this.ro.tupian = true;


			// 下拉框
			res = await this.$api.option(`jiazhaoleixing`,`jiazhaoleixing`,{});
			this.jiazhaoleixingOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`baomingrukou`, this.ruleForm.id);
		...
		methods: {
			...
			// 下拉变化
			jiazhaoleixingChange(e) {
				this.jiazhaoleixingIndex = e.target.value
				this.ruleForm.jiazhaoleixing = this.jiazhaoleixingOptions[this.jiazhaoleixingIndex]
			},
			...
			async onSubmitTap() {
//跨表计算判断
				if(this.ruleForm.xuechefeiyong&&(!this.$validate.isIntNumer(this.ruleForm.xuechefeiyong))){
					this.$utils.msg(`学车费用应输入整数`);
					return
				}
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`baomingrukou`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`baomingrukou`, this.ruleForm);
						}else{
							await this.$api.add(`baomingrukou`, this.ruleForm);
						}
