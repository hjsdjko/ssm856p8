	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`kaoshichangdi`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
//跨表计算判断
				if((!this.ruleForm.kaoshichangdi)){
					this.$utils.msg(`考试场地不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`kaoshichangdi`, this.ruleForm);
						}else{
							await this.$api.add(`kaoshichangdi`, this.ruleForm);
						}
