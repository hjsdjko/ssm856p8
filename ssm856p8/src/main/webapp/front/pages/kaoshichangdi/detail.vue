...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','kaoshichangdi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','kaoshichangdi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('kaoshichangdi', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusskaoshichangdi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('kaoshichangdi', this.detail);
				this.$utils.msg('审核成功');
