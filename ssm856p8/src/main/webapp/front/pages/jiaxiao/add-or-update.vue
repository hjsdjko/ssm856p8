<template>
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">联系人</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiaxiao`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
//跨表计算判断
				if((!this.ruleForm.jiaxiaozhanghao)){
				...
				if((!this.ruleForm.jiaxiaomingcheng)){
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`jiaxiao`, this.ruleForm);
						}else{
							await this.$api.add(`jiaxiao`, this.ruleForm);
						}
