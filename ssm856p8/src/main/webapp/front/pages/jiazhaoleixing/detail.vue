...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiazhaoleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiazhaoleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiazhaoleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiazhaoleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('jiazhaoleixing', this.detail);
				this.$utils.msg('审核成功');
